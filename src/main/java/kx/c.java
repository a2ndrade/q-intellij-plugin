...
public class c {
  ...
  public static class Timespan implements Comparable<Timespan> {
    ...
    public String toString() {
      ...
      return s + i2((int)((jj % 86400000000000L) / 3600000000000L)) + ":" +
          i2((int)((jj % 3600000000000L) / 60000000000L)) + ":" + i2((int)((jj % 60000000000L) / 1000000000L)) + "." +
  ...
  public static int n(Object x) throws UnsupportedEncodingException {
    return x instanceof Dict
        ...
        : x instanceof Flip
            ? n(((Flip)x).y[0])
            : x instanceof char[] ? new String((char[])x).getBytes(encoding).length : Array.getLength(x);
  }
  ...
  public synchronized Object k(Object x) throws KException, IOException {
    ...
    while (true) {
      Object[] msg = readMsg();
      if (msgHandler == null ||
          (byte)msg[0] == (byte)2) // if there's no handler or the msg is a response msg, return it
      {
        return msg[1];
      }
  ...
  public static Object[] NULL = {null, new Boolean(false), new UUID(0, 0), null, new Byte((byte)0),
      new Short(Short.MIN_VALUE), new Integer(ni), new Long(nj), new Float(nf), new Double(nf), new Character(' '), "",
      new Timestamp(nj), new Month(ni), new Date(nj), new java.util.Date(nj), new Timespan(nj), new Minute(ni),
